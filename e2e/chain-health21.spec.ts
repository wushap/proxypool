...
test.describe('Chain Health Check (via Proxy Pools)', () => {
  ...
  test('verify chain flow visualization exists', async ({ page }) => {
    ...
    // Chain visualization container is always rendered (v-show based)
  ...
  test('verify chain node elements exist', async ({ page }) => {
    ...
    // Intermediate nodes (front pool / exit pool) are always rendered in the template
    // Scope to chain-flow to avoid matching elements in the "池类型说明" section below
