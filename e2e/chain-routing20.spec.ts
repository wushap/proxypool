...
test.describe('Chain Routing (Round 20)', () => {
  test('chain view tab exists and is clickable among proxy pool tabs', async ({ page }) => {
    await navigateTo(page, '多跳代理池');

    const tabs = page.locator('.workspace-tabs .tab-btn');
    await expect(tabs.first()).toBeVisible({ timeout: 5000 });

    // Should have at least 5 tabs (pools, chain-view, gateway, gateway-status, chain, backend, events)
  ...
  test('clicking chain view tab activates it and reveals visualization with action buttons', async ({ page }) => {
    await navigateTo(page, '多跳代理池');
  ...
  test('chain flow visualization exists with entry, pools, arrows, and exit nodes', async ({ page }) => {
    await navigateTo(page, '多跳代理池');
    ...
    // All arrows have arrow icon
    for (let i = 0; i < arrowCount; i++) {
      await expect(arrows.nth(i).locator('.chain-arrow-icon')).toContainText('→');
    }
  });

  test('chain node elements display host:port names, status dots, and pool types', async ({ page }) => {
    await navigateTo(page, '多跳代理池');

    await page.locator('.workspace-tabs .tab-btn').filter({ hasText: '链路视图' }).click();
    await page.locator('.chain-flow').waitFor({ state: 'visible', timeout: 5000 });

    // Entry node name should have host:port pattern
    ...
    // Entry node status has "监听中" text
    ...
    // Node types should be labeled for front pool and exit pool
    ...
    // Check that pool type labels are present
...
test.describe('Subscription Intelligence (Round 20)', () => {
  test('subscriptions page shows intelligence panel or empty state when loaded', async ({ page }) => {
  ...
  test('intelligence panel shows multiple analysis cards with titled sections', async ({ page }) => {
    await navigateTo(page, '订阅管理');
  ...
  test('intelligence panel has duplicate node analysis card with badge', async ({ page }) => {
    await navigateTo(page, '订阅管理');
  ...
  test('intelligence panel shows quality scores with per-subscription entries', async ({ page }) => {
    ...
    if (await scoresGrid.isVisible()) {
      ...
      // Each entry should show a numeric score and node/reliability info
...
test.describe('System Diagnostics Health Score (Round 20)', () => {
  ...
  test('running diagnostics completes and health score appears in valid range', async ({ page }) => {
    ...
    // Diagnostic details should appear
