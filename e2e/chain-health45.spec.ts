...
// ── Chain Health Check (Round 45) ──

test.describe('Chain Health Check (Round 45)', () => {
  ...
  test('dashboard has protocol distribution', async ({ page }) => {
    ...
    const card = protocolTitle.locator('..').locator('..');
    const donutChart = card.locator('.dashboard-donut-wrapper, .dashboard-donut-chart');
    const emptyState = card.locator('.empty-state');
...
// ── Batch Operations (Round 45) ──

test.describe('Batch Operations (Round 45)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');
...
// ── System Diagnostics Export (Round 45) ──

test.describe('System Diagnostics Export (Round 45)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');
  ...
  test('publish page has create form', async ({ page }) => {
    const createTitle = page.locator('.settings-title, h3').filter({ hasText: '创建发布订阅' });
    await expect(createTitle.first()).toBeVisible();
