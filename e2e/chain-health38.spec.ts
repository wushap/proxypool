...
// ── Chain Health Check (Round 38) ──

test.describe('Chain Health Check (Round 38)', () => {
...
// ── Batch Operations (Round 38) ──

test.describe('Batch Operations (Round 38)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');
  ...
  test('subscription page has group tabs', async ({ page }) => {
    // Group tabs may only appear after subscriptions load; check for either the tab bar or the add-form area
  ...
  test('subscription page has batch buttons', async ({ page }) => {
    // Section header contains management buttons: 刷新全部, 删除不可用, 刷新列表
    const sectionHeader = page.locator('.section-header');
    ...
    // Bulk import detail exists on the page
