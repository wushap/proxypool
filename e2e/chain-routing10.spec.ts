...
test.describe('Chain Routing (Round 10)', () => {
  ...
  test('clicking chain view tab loads chain visualization content', async ({ page }) => {
    ...
    // Chain visualization header and hint text should appear
  ...
  test('chain flow visualization has entry node with status indicator', async ({ page }) => {
    await navigateTo(page, '多跳代理池');
    ...
    // Entry node should have a status dot and status text
    await expect(entryNode.locator('.status-dot').first()).toBeVisible();

    // At least one arrow should connect entry to the next node
    const arrows = page.locator('.chain-flow .chain-arrow');
    const arrowCount = await arrows.count();
    expect(arrowCount).toBeGreaterThanOrEqual(1);
  });

  test('chain view has action buttons for diagnostics and testing', async ({ page }) => {
    await navigateTo(page, '多跳代理池');

    await page.locator('.tab-btn').filter({ hasText: '链路视图' }).click();
    await page.locator('.chain-flow').waitFor({ state: 'visible', timeout: 5000 });

    // Verify action buttons exist in the chain view section header
    ...
    await expect(latencyButton).toBeEnabled();

    const fullChainButton = page.locator('button:has-text("测试整条链路")');
    await expect(fullChainButton).toBeVisible();
    await expect(fullChainButton).toBeEnabled();
  });
});

// ── Subscription Intelligence (via Subscriptions page) ──

test.describe('Subscription Intelligence (Round 10)', () => {
  ...
  test('quality scores card displays score entries with numeric values', async ({ page }) => {
    await navigateTo(page, '订阅管理');
    ...
    if (await scoresGrid.isVisible()) {
      ...
      // First score entry should contain a numeric value
...
test.describe('System Diagnostics Health Score (Round 10)', () => {
  ...
  test('run diagnostics and verify health score appears with valid range', async ({ page }) => {
    await navigateTo(page, '系统诊断');
  ...
  test('run diagnostics and verify detailed report sections appear', async ({ page }) => {
    await navigateTo(page, '系统诊断');
    ...
    // Detailed report sections should appear after running
    const diagnosticSections = page.locator('.diagnostic-section-title');
    const sectionCount = await diagnosticSections.count();
    expect(sectionCount).toBeGreaterThanOrEqual(2);

    // Backend status section should show key metrics
    await expect(page.locator('.diagnostic-label:has-text("运行状态")').first()).toBeVisible();

    // Export button should become enabled after report is generated
