...
test.describe('Chain Routing (Round 23)', () => {
  ...
  test('clicking chain view tab activates it and loads visualization header', async ({ page }) => {
    await navigateTo(page, '多跳代理池');
  ...
  test('chain flow visualization container exists with entry and exit nodes', async ({ page }) => {
    ...
    // Entry node (first node in the chain)
    const entryNode = page.locator('.chain-node-entry').first();
    await expect(entryNode).toBeVisible();

    // Exit point (output node, last in the chain)
    const exitNode = page.locator('.chain-node-exit').first();
    await expect(exitNode).toBeVisible();
  });

  test('chain node elements display type labels, arrows, and status indicators', async ({ page }) => {
    await navigateTo(page, '多跳代理池');
    ...
    // Verify chain arrows exist (entry -> front pool -> exit pool -> output)
    ...
    // Verify pool type labels exist (入口, 前置池, 落地池, 出口)
...
test.describe('Subscription Intelligence (Round 23)', () => {
  ...
  test('intelligence panel shows duplicate node analysis with status badge', async ({ page }) => {
    ...
    // Content should mention duplicates or no duplicates
  ...
  test('intelligence panel shows quality scores with per-subscription entries', async ({ page }) => {
    ...
    if (await scoresGrid.isVisible()) {
      ...
      // Each entry should show node count info
...
test.describe('System Diagnostics Health Score (Round 23)', () => {
