...
router = APIRouter(prefix="/api", tags=["tester"])
...
async def run_tester(
    ...
    report = await tester.run_batch(
        ...
        only_available=body.get("only_available", False),
        only_unavailable=body.get("only_unavailable", False),
        min_last_checked_age_hours=body.get("min_last_checked_age_hours"),
        protocols=body.get("protocols"),
        fallback_front_proxy_keys=body.get("fallback_front_proxy_keys"),
        fallback_front_max_attempts=body.get("fallback_front_max_attempts"),
        replace_failed_with_available=body.get("replace_failed_with_available", False),
    )
    return asdict(report)


@router.post("/tester/run-one")
async def run_single_proxy_test(
    ...
    tester = request.app.state.tester
    key = str(body.get("normalized_key") or "").strip()
    if not key:
...
@router.post("/tasks/tester/start")
...
@router.post("/tasks/speed-test/start")
async def start_speed_test_task(
    ...
    if not (target_url.startswith("http://") or target_url.startswith("https://")):
        raise HTTPException(status_code=400, detail="speed test url must start with http:// or https://")

    def _runner(update, should_stop):
        only_direct_raw = body.get("only_direct")
        only_direct = bool(body.get("only_available")) if only_direct_raw is None else bool(only_direct_raw)
        ...
        for idx, node in enumerate(candidates, start=1):
            ...
            result = asyncio.run(
                tester.prober.speed_test_async(
                    node,
                    target_url,
                    timeout_sec=float(body.get("timeout_sec") or 30.0),
                )
            )
            item = {
                "normalized_key": key,
