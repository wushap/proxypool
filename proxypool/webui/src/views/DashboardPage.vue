  ...
  <section v-show="activePage === 'dashboard'" class="dashboard-page fade-in">
    ...
    <div class="stat-grid dashboard-stat-grid">
      <div class="card stat-card">
        <div class="stat-card-label">节点总数</div>
        <div class="stat-card-value">{{ stats.total ?? 0 }}</div>
        <div class="stat-card-desc">已收录代理节点</div>
      </div>
      <div class="card stat-card">
        <div class="stat-card-label">可用节点</div>
        <div class="stat-card-value" style="color: var(--success-text);">{{ stats.available ?? 0 }}</div>
        <div class="stat-card-desc">通过可用性测试</div>
      </div>
      <div class="card stat-card">
        <div class="stat-card-label">可用率</div>
        <div class="stat-card-value">{{ (stats.availability_rate ?? 0) }}%</div>
      ...
      <div class="card stat-card">
        ...
        <div class="stat-card-value">{{ stats.avg_latency_ms ? stats.avg_latency_ms + 'ms' : '-' }}</div>
        <div class="stat-card-desc">可用节点平均值</div>
      </div>
    </div>
    ...
    <div class="stat-grid" style="grid-template-columns: repeat(3, 1fr); margin-bottom: 12px;">
      <div class="card stat-card">
        <div class="stat-card-label">平均带宽</div>
        <div class="stat-card-value">{{ stats.avg_speed_mbps ? stats.avg_speed_mbps + ' Mbps' : '-' }}</div>
        <div class="stat-card-desc">可用节点平均下载速度</div>
      </div>
      <div class="card stat-card">
        <div class="stat-card-label">ChatGPT 解锁</div>
        <div class="stat-card-value" style="color: var(--success-text);">{{ stats.openai_unlocked ?? 0 }}</div>
        <div class="stat-card-desc">
      ...
      <div class="card stat-card">
        <div class="stat-card-label">订阅源</div>
        <div class="stat-card-value">{{ stats.subscription_count ?? subscriptions.length ?? 0 }}</div>
        <div class="stat-card-desc">已配置的订阅源</div>
      </div>
    </div>
    ...
    <div class="dashboard-grid">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div v-if="protocolEntries.length" class="dashboard-protocol-list">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div v-if="countryEntries.length" class="dashboard-protocol-list">
          ...
          <div v-else class="empty-state">暂无国家数据（请先运行 IP 位置补全）</div>
        </div>
      </div>
    </div>
    ...
    <div class="dashboard-grid" style="margin-top: 12px;">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div v-if="purityEntries.length" class="dashboard-protocol-list">
          ...
          <div v-else class="empty-state">暂无纯净度数据（请先运行 IP 纯净度检测）</div>
        </div>
      </div>
    ...
    <div class="card" style="margin-top: 12px;">
      <div class="card-body">
        ...
        <div v-if="recentTasks.length" class="dashboard-task-list">
...
export default {
  ...
  watch: {
    activePage(val) {
      if (val === 'dashboard' && this.autoRefreshSec > 0) {
